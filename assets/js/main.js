(function ($) {
    ...
    $body.on('click', ".js-ajax-link:not([href*='://'])", function(e){
        ...
        else if(!$(this).parent('li').hasClass('active') || $(this).parents('#nav').length && $(this).next('ul').find('.active').length){
                ...
                setTimeout(function(){
                    ...
                    if($('.contacts-map').length){
                        if(!$('#ggl-map-js').length){
                            $body.append('<script src="https://maps.googleapis.com/maps/api/js?v=3&amp;key=AIzaSyAh-ck8YK2xAhhQgKhEyWdowVKrHd9SjI4&amp;libraries=places" id="ggl-map-js"></script>');
                        }
